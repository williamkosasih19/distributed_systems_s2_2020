  ...
  public void run() {
    ...
    try {
      outputFileWriter = new PrintWriter("M" + 
                                         myClientNumber.toString() + "_TIME.log");
    ...
    while (true) {
      ...
      date = new Date();
      long firstTimeTick = date.getTime();
      try {
        while (true) {
          ...
          if (proposedMessageCount > numberOfClients / 2) {
            ...
            date = new Date();
            long secondTimeTick = date.getTime();
            Long timeElapsed = secondTimeTick - firstTimeTick;
            System.out.println("Time elapsed: " + 
                               timeElapsed.toString());
            
            System.out.println(statusString);
            // outputFileWriter.println(statusString);
            outputFileWriter.println(timeElapsed.toString());
